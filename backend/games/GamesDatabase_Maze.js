...
db.prepare(`
    CREATE TABLE IF NOT EXISTS maze_times (
...
db.prepare(`
    CREATE TABLE IF NOT EXISTS maze_data (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        gameBoard TEXT,
        dateString TEXT,
        averageTime REAL
...
const getEntriesStatement = db.prepare(`SELECT * FROM maze_times`);
...
// Return all info for the leaderboard from the maze database
const getTodaysEntriesStatement = db.prepare(`SELECT * FROM maze_times WHERE dateString=@dateString`);
const getLeaderboardEntriesStatement = db.prepare(`SELECT * FROM maze_times WHERE topTen='true'`);
const getAverageTimeStatement = db.prepare(`SELECT averageTime, dateString FROM maze_data ORDER BY id DESC LIMIT 30`);
...
const addEntryStatement_maze = db.prepare(`INSERT INTO maze_times (name, time, dateString, topTen, placing) VALUES (@name, @time, @dateString, @topTen, @placing)`);
const updateEntryStatement_maze = db.prepare(`UPDATE maze_times SET topTen=@topTen, placing=@placing WHERE id=@id`);
const updateEntryStatement_maze_data = db.prepare(`UPDATE maze_data SET averageTime=@averageTime WHERE dateString=@dateString`);
async function addTimeEntry(playData){
    ...
    try{ addEntryStatement_maze.run(Object.assign(playData, { topTen: 'true', placing: ALLTIME_LEADERBOARD_COUNT+1 })); }
    ...
    try{
        ...
        for(const [i, timeEntry] of currentAllTimeBest.entries()){
            ...
            updateEntryStatement_maze.run(timeEntry);
        }
    }
    ...
    try{
        ...
        updateEntryStatement_maze_data.run(updateObj);
    }
    catch(err){
...
const getEntryStatement_maze_data = db.prepare(`SELECT * from maze_data WHERE dateString=@dateString`);
const addEntryStatement_maze_data = db.prepare(`INSERT INTO maze_data (gameBoard, dateString, averageTime) VALUES (@gameBoard, @dateString, @averageTime)`);
async function addNewGameBoard(gameBoard){
    ...
    try{ // only add a new one if its not already added, this supports project stopping and starting whenever
        const currentEntry = getEntryStatement_maze_data.all(databaseObj);
        if(!currentEntry.length){ addEntryStatement_maze_data.run(databaseObj); }
    }
    catch(err){ return { success: false, error: `Error inserting into database: ${err}` }; }
...
const getSingleEntryFromId = db.prepare(`SELECT * FROM maze_times WHERE id=@id`);
const deleteTodaysEntryStatement = db.prepare(`DELETE FROM maze_times WHERE id=@id`);
const getFastestEntries = db.prepare(`SELECT * FROM maze_times ORDER BY time ASC LIMIT ${ALLTIME_LEADERBOARD_COUNT+5}`);
    ...
    try{
        ...
        if(entryToDelete.topTen === 'true'){
            ...
            for(const [i, entry] of entires.entries()){
                ...
                updateEntryStatement_maze.run(entry);
            }
        }

        return { success: true };
    }
    catch(err){ return { success: false, error: 'Error getting entries from \'maze\' database' }; }
};
